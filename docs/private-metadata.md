...
- messaging contacts (Reminders' "Remind me when messaging"): `edit 23880 --private --remind-when-messaging "+15555550123,alex@example.com"`
...
`--remind-when-messaging` is the Reminders "Remind me when messaging" trigger. It splits its comma-separated value into phone and email handles (anything containing `@` is treated as an email) and validates that at least one handle is present before saving. EventKit has no messaging trigger, so this only goes through `remctl-private`. Reminders stores the trigger on the reminder itself, not as an alarm: `remctl-private` sets the reminder's `contactHandles` (a `REMContactRepresentation` built from the phone and email handles) through `REMReminderChangeItem`, which Reminders persists to `ZREMCDREMINDER.ZCONTACTHANDLES`. `info --json` reports it under `contactHandles` with `phones` and `emails` arrays. Pass `clear` as the value to remove the trigger (`remctl-private` calls `setContactHandles:nil`).
