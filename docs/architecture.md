...
`tag-rename` and `tag-delete` are private. Tags are account-level `REMHashtagLabel` rows referenced by per-reminder `REMHashtag` objects, so there is no rename-label API: Python reuses the tag-read join to enumerate every reminder carrying the tag, then fans out a `rewrite_reminder_hashtags` write per reminder. The helper reads the reminder's current hashtags through `REMReminderHashtagContext`, matches by name, and calls `removeHashtag:` and/or `addHashtagWithType:name:` on the reminder change item before saving. Python then rewrites any custom smart-list `filterData` that references the tag and writes it back through `update_smart_list` to keep Reminders' filter cache consistent.
