...
- `ZSORTINGSTYLE` stores the Reminders.app sort order for a list or smart list. Reminders stores `manual` and `default` bare; `priority`, `title`, `displayDate`, and `creationDate` carry a direction suffix (`priority_desc`, `title_asc`, `displayDate_asc`, `creationDate_asc` for the Reminders.app defaults); empty means the Reminders.app default.
