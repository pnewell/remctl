...
| Default list for new reminders | `default-list <list> --private` | Yes | `default-list --show` |
...
- Commands that target lists consistently support exact numeric targeting where the underlying write/read path is safe: `show --list-id`, `add --list-id`, `edit --list-id`, `link --list-id`, `export --list-id`, `list-edit --list-id`, `list-pin --list-id`, `list-unpin --list-id`, `default-list --list-id`, `list-rename --list-id --new-name`, `list-delete --list-id`, plus smart-list `--include-list-id`. `list-pin` and `list-unpin` also accept smart-list names or `--smart-list-id`.
...
remctl default-list "Projects" --private --json
remctl default-list --show --json
remctl group-create "Writing" --private --add-list Editorial --json
